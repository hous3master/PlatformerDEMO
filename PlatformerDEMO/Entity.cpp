...
void Entity::new_orientation(direction new_data) {
	...
	if (orientation == direction::up) { dy = -vertical_velocity; }
	if (orientation == direction::left) { dx = -horizontal_velocity; current_row = 1; }
	if (orientation == direction::right) { dx = horizontal_velocity; current_row = 0; }

	if (orientation == direction::still) { dx = 0; dy = 0; }
}
